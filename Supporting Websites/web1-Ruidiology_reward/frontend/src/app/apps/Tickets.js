...
function Tickets() {
  ...
  useEffect(() => {
    ...
    fetch(
      `http://172.22.215.62:5000/descr?low=${context.user.low_index}&high=${context.user.high_index}`
    )
      .then((res) => res.json())
      .then((tickets) => {
  ...
  const handleSubmit = (index) => {
    ...
    // );
    fetch(
      `http://172.22.215.62:5000/descr/updtt?idx=${activeTickets[index].idx}&best=${activeTicketBestStates[index]}&worst=${activeTicketWorstStates[index]}`,
      { method: "PUT" }
    ).then((res) => res.json());
  ...
  const handleRevoke = (index) => {
    fetch(
      `http://172.22.215.62:5000/descr/activate?idx=${inactiveTickets[index].idx}`,
      { method: "PUT" }
    ).then((res) => res.json());
