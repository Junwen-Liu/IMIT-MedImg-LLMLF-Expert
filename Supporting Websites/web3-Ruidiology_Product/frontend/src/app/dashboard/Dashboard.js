...
const Dashboard = () => {
  ...
  useEffect(() => {
    fetch(`http://172.22.16.34:5001/login/allUsers`)
      .then((res) => res.json())
      .then((users) => {
        setAllUsers(users);
        ...
        users.map((user) => {
          try {
            fetch(
              `http://localhost:5001/descr/countUser?high=${user.high_index}&low=${user.low_index}`
            )
              .then((res) => res.json())
              .then(({ doneTickets }) => {
                // return doneTickets;
    ...
    fetch(`http://localhost:5001/descr/countAll`)
      .then((res) => res.json())
