...
function Tickets() {
  ...
  const handleSubmit = (index) => {
    ...
    // );
    fetch(
      `http://127.0.0.1:5001/descr/updtt?idx=${activeTickets[index].idx}&best=${activeTicketBestStates[index]}&worst=${activeTicketWorstStates[index]}`,
      { method: "PUT" }
    ).then((res) => res.json());
  ...
  const handleRevoke = (index) => {
    fetch(
      `http://127.0.0.1:5001/descr/activate?idx=${inactiveTickets[index].idx}`,
      { method: "PUT" }
    ).then((res) => res.json());
