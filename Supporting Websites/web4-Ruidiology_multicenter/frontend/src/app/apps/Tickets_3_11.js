...
function Tickets() {
  ...
  useEffect(() => {
    if (runCount.current < 1) {
      ...
      fetch(
        `http://127.0.0.1:5001/descr?low=${context.user.low_index}&high=${context.user.high_index}`
      )
        .then((res) => res.json())
        .then((tickets) => {
          setRecords(() => {
  ...
  const handleSubmit = (index) => {
    ...
    fetch(
      `http://127.0.0.1:5001/descr/updttnew?idx=${activeTickets[index].idx}&A=${activeTickets[index]['A']}&C=${activeTickets[index]['C']}&H=${activeTickets[index]['H']}&M=${activeTickets[index]['M']}&R=${activeTickets[index]['R']}`,
      { method: "PUT" }
    ).then((res) => res.json());
  ...
  const handleRevoke = (index) => {
    fetch(
      `http://127.0.0.1:5001/descr/activatenew?idx=${inactiveTickets[index].idx}`,
      { method: "PUT" }
    ).then((res) => res.json());
