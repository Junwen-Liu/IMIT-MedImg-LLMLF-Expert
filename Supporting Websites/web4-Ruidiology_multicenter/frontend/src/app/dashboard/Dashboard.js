...
const Dashboard = () => {
  ...
  useEffect(() => {
    fetch(`http://220.196.190.94:8001/login/allUsers`)
      .then((res) => res.json())
      .then((users) => {
        setAllUsers(users);
        ...
        users.map((user) => {
          try {
            fetch(
              `http://220.196.190.94:8001/descr/countUser?high=${user.high_index}&low=${user.low_index}`
            )
              .then((res) => res.json())
              .then(({ doneTickets }) => {
                // return doneTickets;
    ...
    fetch(`http://220.196.190.94:8001/descr/countAll`)
      .then((res) => res.json())
          ...
          <div className="card bg-gradient-info card-img-holder text-white">
            <div className="card-body">
              ...
              <h4 className="font-weight-normal mb-3">
                Attending Verification Centers{" "}
                <i className="mdi mdi-diamond mdi-24px float-end"></i>
              </h4>
              <h2 className="mb-5">8 Hospitals, thank you SO MUCH!</h2>
        ...
        <div className="col-md-4 stretch-card grid-margin">
          <div className="card bg-gradient-primary card-img-holder text-white">
            <div className="card-body">
              ...
              <h2 className="mb-5">{totalCount} Cases</h2>
        ...
        <div className="col-md-4 stretch-card grid-margin">
          <div className="card bg-gradient-success card-img-holder text-white">
            <div className="card-body">
              ...
              <h2 className="mb-5">{allusers.length} Radiologists</h2>
              <h6 className="card-text"></h6>
            </div>
          </div>
        </div>
                  ...
                  {allusers.map((user) => {
                    ...
                    return (
                      <tr key={user.username}>
                        ...
                        <td style={{ color: '#0B71FA', fontWeight: 'bold' }}> {user.username} </td>
                        <td style={{ color: '#AD66FC', }}> {user.position} </td>
                        <td style={{ color: '#0B71FA', }}> {user.site} </td>
                        <td>
                          <ProgressBar
                            variant="success"
                            now={
                        ...
                        <td style={{ color: '#0B71FA' }}> {totalCount} </td>
                        {/* <td> May 15, 2015 </td> */}
                      </tr>
                    );
                  })}
